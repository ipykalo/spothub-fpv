...
import { type CandidateFlight, TRACK_SEARCH_WINDOW_MS, matchTrack } from './track-matcher';
...
const RADIO_FLIGHT = flight('air65', '2026-09-14T18:32:10.000Z', '2026-09-14T18:34:12.800Z');
const GPX_TRACK = { startedAt: at('2026-09-14T16:32:13.000Z'), endedAt: at('2026-09-14T16:34:12.800Z') };

describe('matchTrack', () => {
  it('joins a flight whose times overlap the track as recorded', () => {
    const track = { startedAt: at('2026-09-14T18:32:13.000Z'), endedAt: at('2026-09-14T18:34:12.800Z') };
  ...
  it('finds a flight from a radio set to local time, two hours ahead of UTC', () => {
    expect(matchTrack(GPX_TRACK, [RADIO_FLIGHT])).toEqual({ flightId: 'air65', offsetMs: 2 * HOUR });
  });
  ...
  it('finds a flight from a radio behind UTC, and from a half-hour zone', () => {
    ...
    expect(matchTrack(GPX_TRACK, [newYork])).toEqual({ flightId: 'ny', offsetMs: -4 * HOUR });
    expect(matchTrack(GPX_TRACK, [india])).toEqual({ flightId: 'in', offsetMs: 5.5 * HOUR });
  });

  it('prefers the time as recorded over a shifted one', () => {
    const recorded = flight('same', '2026-09-14T16:32:05.000Z', '2026-09-14T16:34:20.000Z');

    expect(matchTrack(GPX_TRACK, [RADIO_FLIGHT, recorded])).toEqual({ flightId: 'same', offsetMs: 0 });
  });
